...
def dehaze_paired_paths_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    for idx in range(len(input_paths)):
        ...
        gt_path = input_path.split('_')[0] + '.jpg'
        assert input_name in input_paths, (f'{input_name} is not in '
                                           f'{input_key}_paths.')
        
        input_path = osp.join(input_folder, input_path)
        gt_path = osp.join(gt_folder, gt_path)
        # print("input_path, gt_path:", input_path, gt_path)
        paths.append(
            dict([(f'{input_key}_path', input_path),
                  (f'{gt_key}_path', gt_path)]))
    return paths
